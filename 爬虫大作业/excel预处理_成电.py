...
for line in csv_r:
    ...
    # 添加浏览次数
    views_mo=views_re.search(line[3])
    views_list.append(int(views_mo.group()))

#创建表格
workbook=xw.Workbook("CD_D.xlsx")
worksheet1=workbook.add_worksheet("成电")
worksheet1.activate()
...
for i in range(length):
    ...
    title_list[i].strip()
    # 插入数据
    insert_row=[number_list[i],title_list[i],None,date_list[i],views_list[i]]
    row='A'+str(j)
